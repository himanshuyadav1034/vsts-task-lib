...
function Get-TaskVariable {
    ...
    try {
        ...
        if ($script:variableInfo.$variableKey.Secret) {
            # Get secret variable.
...
<#
.SYNOPSIS
Gets all task variables. Requires 2.104.1 agent or higher.

.DESCRIPTION
Gets all task variables. Returns an array of objects with the following properties:
    [string]Name
    [string]Value
    [bool]Secret
#>
function Get-TaskVariableInfo {
    ...
    foreach ($info in $script:variableInfo.Values) {
        New-Object -TypeName psobject -Property @{
            Name = $info.Name
...
function Set-TaskVariable {
    ...
    $variableKey = Get-VariableKey -Name $Name
    if ($Secret -or $script:variableInfo.$variableKey.Secret) {
        $vaultKey = "SECRET_$variableKey"
        if (!$Value) {
            # Clear the secret.
            Write-Verbose "Set $vaultKey = ''"
        ...
        Write-Verbose "Set $variableKey = '$Value'"
...
function Initialize-Inputs {
    ...
    foreach ($variable in (Get-ChildItem -Path Env:ENDPOINT_?*, Env:INPUT_?*, Env:SECRET_?*)) {
        ...
        if ($variable.Name -like 'SECRET_?*') {
            ...
            $script:variableInfo[$variableKey] = New-Object -TypeName psobject -Property @{
